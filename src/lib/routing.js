...
const FLOOR_ORDER = ['G', 'UG', 'F1', 'F2', 'F3']
...
export const floorLabelOf = (floorId) => FLOORS.find((f) => f.id === floorId)?.label ?? floorId
...
export function describeRoute(startId, endId) {
  ...
  steps.push(`Start at ${start.name}, ${floorLabelOf(start.floor)}.`)
  ...
  while (i < path.length - 1) {
    ...
    if (here.floor !== next.floor) {
      ...
      let j = i + 1
      while (j < path.length - 1 && path[j + 1].floor !== path[j].floor && path[j].type === 'atrium') j++
      ...
      escalatorHops += hops
      steps.push(
        `Take the escalator at ${here.name} ${up ? 'up' : 'down'}${
          hops > 1 ? ` ${hops} floors` : ''
  ...
  if (dest.type === 'store') {
    ...
    if (neighbour) steps.push(`You'll find it right next to ${neighbour.name}.`)
  }

  const minutes = Math.max(1, Math.round(walkMetres / 75 + escalatorHops * 0.5))
  return { steps, minutes, metres: Math.round(result.metres), dest }
}
